...
getInput()
  ...
  .forEach((line, index) => {
    ...
    if (index % 3 === 2) {
      const split = rucksacks.map((rucksack) => {
        return Array.from(new Set(rucksack.split('')));
      });

      const intersect = split[0]
        .filter((item) => {
          return split[1].includes(item);
        })
        .filter((item) => {
          return split[2].includes(item);
        });

      prioritySum += getCharCode(intersect[0]);
